#!/usr/bin/env python3
"""
tools/read_innova_basic_g2.py
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
PHOENIXTEC Innova Basic G2 USB HID UPS Live Reader.
Uses core_hid_ups engine to read dynamic feature reports and telemetry.

Usage:
    python tools/read_innova_basic_g2.py
"""
...
def read_innova_basic_g2() -> None:
    print("=" * 76)
    print(" 🔌 PHOENIXTEC Innova Basic G2 — USB HID UPS Live Reader")
    print(" Profile: phoenixtec_innova_basic_g2")
    print("=" * 76)

    # Find connected Innova Basic G2
    devices = list_ups_devices(target_vid=0x06DA)
    target = None
    for dev in devices:
        p_str = (dev.get("product_string") or "").lower()
        s_num = dev.get("serial_number") or ""
        if "basic" in p_str or dev.get("profile_id") == "phoenixtec_innova_basic_g2" or "CPLUV" in s_num:
            target = dev
            break

    if not target:
        print("\n❌ ไม่พบอุปกรณ์ PHOENIXTEC Innova Basic G2 (VID=0x06DA, PID=0xFFFF)")
    ...
    try:
        ...
        print("=" * 76)
        print(" 📊 Real-Time Telemetry (Innova Basic G2)")
        print("=" * 76)
