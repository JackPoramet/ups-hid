#!/usr/bin/env python3
"""
tools/unit/test_2000d_exact_8byte_feature.py
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
ค้นพบจุดสำคัญ: PPC 2000D มี FeatureReportByteLength = 8 Bytes เป๊ะๆ (ไม่มี Output Report)
ทดสอบส่ง HidD_SetFeature ด้วย Buffer ขนาด 8 Bytes ทุกรูปแบบ Payload เพื่อหาโครงสร้างที่กระตุ้น Relay และ Beep จริง
"""
...
# เปิดด้วย Query Access Mode (dwDesiredAccess = 0)
...
print("✅ เปิด Handle สำเร็จ! เริ่มส่ง HidD_SetFeature ( Buffer ขนาด 8 Bytes เป๊ะๆ )...\n")

test_payloads = [
    ("RID 0x00 Q1 'T\\r' [0x00, 'T', '\\r', 0..]", bytes([0x00, ord('T'), ord('\r'), 0, 0, 0, 0, 0])),
    ("RID 0x00 Q1 'T' [0x00, 'T', 0..]", bytes([0x00, ord('T'), 0, 0, 0, 0, 0, 0])),
    ("RID 0x00 Code [0x00, 0x03, 0x01, 0..]", bytes([0x00, 0x03, 0x01, 0, 0, 0, 0, 0])),
    ("RID 0x00 Code [0x00, 0x01, 0x00, 0..]", bytes([0x00, 0x01, 0x00, 0, 0, 0, 0, 0])),
    ("RID 0x01 Q1 'T\\r' [0x01, 'T', '\\r', 0..]", bytes([0x01, ord('T'), ord('\r'), 0, 0, 0, 0, 0])),
    ("RID 0x01 Code [0x01, 0x01, 0..]", bytes([0x01, 0x01, 0, 0, 0, 0, 0, 0])),
    ("RID 0x02 Q1 'T\\r' [0x02, 'T', '\\r', 0..]", bytes([0x02, ord('T'), ord('\r'), 0, 0, 0, 0, 0])),
    ("RID 0x02 Code [0x02, 0x01, 0..]", bytes([0x02, 0x01, 0, 0, 0, 0, 0, 0])),
    ("RID 0x03 Code [0x03, 0x01, 0..]", bytes([0x03, 0x01, 0, 0, 0, 0, 0, 0])),
    ("RID 0x03 Q1 'T\\r' [0x03, 'T', '\\r', 0..]", bytes([0x03, ord('T'), ord('\r'), 0, 0, 0, 0, 0])),
    ("RID 0x24 Code [0x24, 0x01, 0..]", bytes([0x24, 0x01, 0, 0, 0, 0, 0, 0])),
]
